...
TokenType = enum.Enum("TokenType", "text form lemma paradigm all")
...
def load_tokens():
    ...
    token_data[token_type.paradigm] = {}

    with open(os.path.join(os.path.dirname(__file__), "tokens.txt"), encoding="UTF-8") as f:
        for line in f:
            ...
            token_data[TokenType.all].append([text, form, pos, tag1, lemma])
            if lemma in token_data[TokenType.paradigm]:
                if not pos in token_data[TokenType.paradigm]:
                    token_data[TokenType.paradigm][lemma][tag1] = form
            else:
                token_data[TokenType.paradigm][lemma] = {tag1 : form}
